...
class AgencyTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(5):
            actor = Actor(name=self.new_actor['name'], age=self.new_actor['age'], gender=self.new_actor['gender'])
            actor.insert()
        for i in range(5):
            movie = Movie(title=self.new_movie['title'], release=self.new_movie['release'])
            movie.insert()
        
        self.director_token = os.getenv('CASTING_DIRECTOR')
        self.producer_token = os.getenv('EXECUTIVE_PRODUCER')
    ...
    def test_creating_movie_success(self):
        ...
        response = self.client().post('/movies',
                    headers=self.producer_header(), json=self.new_movie)
    ...
    def test_creating_movie_failure(self):
        # trying to create movie with DIRECTOR TOKEN, where he does not have permissions
        response = self.client().post('/movies',
                    headers=self.director_header(), json=self.new_movie)
        data = json.loads(response.data)
    ...
    def test_delete_movie_failure(self):
        # trying to delete movie with DIRECTOR TOKEN, where he does not have permissions
        movie = Movie.query.all()[0]
        response = self.client().delete('/movies/{}'.format(movie.id),
    ...
    def test_creating_actor_success(self):
        ...
        response = self.client().post('/actors',
                    headers=self.producer_header(), json=self.new_actor)
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['success'], True)
